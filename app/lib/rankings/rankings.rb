module Rankings
  ...
  def Rankings.ranking_2_backtest
    results = {}
    Stock.with_pisbs.each do |stock|
      ...
      real_growth = ( (stock.prices.last.close / stock.latest_price) - 1).round(2)
      results.store(stock.ticker, [target_price, growth_potential, stock, fair_value_combine_data, real_growth])
    end

    file = File.open("./ranking_2_backtest", "w")
    file.truncate 0
    file.write "ticker , target_price , growth_potential , comment , rating , average_net_profit_yearly(3) , cash-debt , market_cap , real_growth\n"
    ...
    sorted.each do |arr|
      ...
      real_growth = arr[1][4]
      if arr[1][0] > 0
        file.write "#{arr[0]} , #{arr[1][0]} , #{arr[1][1]} , #{stock.comment} , #{stock.rating} , #{data[:future_profit]} , #{data[:cash] - data[:debt]} , #{data[:market_cap]} , #{real_growth}\n"
      end
    end
